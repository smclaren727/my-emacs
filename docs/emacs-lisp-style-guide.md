...
based on my extensive experience of using Emacs and creating/maintaining
Emacs packages, feedback and suggestions from members of the Emacs Lisp
community, and various highly regarded Emacs Lisp programming resources,
such as ["GNU Emacs Lisp Reference
...
Please note that the Emacs developers maintain a list of [coding
conventions and
tips](http://www.gnu.org/software/emacs/manual/html_node/elisp/Tips.html#Tips)
too.
...
- If any text precedes an opening bracket (`(`, `{` and `[`) or follows
  a closing bracket (`)`, `}` and `]`), separate that text from that
...
- Do not place blank lines in the middle of a function or macro
  ...
  ;; good - grouping pairwise constructs in let
  (defun bar ()
    (let ((x 1)
          (y 2)
...
- Avoid parameter lists with more than three or four positional
  parameters. Long positional parameter lists are hard to remember and
  easy to get wrong at call sites – callers have to recall the exact
  order, and it's unclear what each argument means without checking the
  definition. Consider using keyword arguments via `cl-defun` instead.
  Note that enabling `eldoc-mode` in Emacs alleviates this issue to a
  large extent, as you constantly see the signature of a function in the
  minibuffer.

  ``` commonlisp
  ;; good - use keyword arguments via cl-defun
  (cl-defun create-buffer (&key name mode read-only)
    ...)
  (create-buffer :name "*foo*" :mode 'text-mode :read-only t)

  ;; good - or use an alist/plist for configuration
  (defun create-buffer (config)
    (let-alist config
      ...))
  (create-buffer '((name . "*foo*") (mode . text-mode) (read-only . t)))

  ;; bad - too many positional parameters
  (defun create-buffer (name mode read-only)
    ...)
  (create-buffer "*foo*" 'text-mode t) ; what do these mean?
  ```

- Always enable [lexical
  scoping](https://www.gnu.org/software/emacs/manual/html_node/elisp/Lexical-Binding.html).
  Lexical scoping reduces the risk of unintended variable capture, makes
  closures work as expected, and enables additional byte-compiler
  optimizations. There's rarely a reason to use dynamic scoping for new
  code. This must be done on the first line as a file local variable.
...
- Use `not` instead of `null`, unless you're checking whether something
  is `nil` (empty list). Despite its name (`null` instead of `nullp`),
  the function `null` is mostly meant to be used as a predicate.
...
- Face names [should
  not](https://www.gnu.org/software/emacs/manual/html_node/elisp/Defining-Faces.html)
  end in `-face`.
...
- Don't write a macro if a function will do. Macros don't compose like
  functions – you can't `funcall`, `apply`, or pass them as arguments –
  so they should only be used when you actually need compile-time code
  transformation.

``` commonlisp
;; good - a function is sufficient here
(defun double (x)
  (* x 2))

;; bad - a macro is unnecessary when a function works just as well
(defmacro double (x)
  `(* ,x 2))
```

- Create an example of a macro usage first and the macro afterwards.
  Starting with the desired usage makes it easier to design a natural
  API and avoids over-engineering the macro's interface.

- Break complicated macros into smaller functions whenever possible.
  Extracting logic into functions makes it easier to test and debug,
  since you can call and inspect functions at runtime but not macro
  internals.

``` commonlisp
;; good - macro delegates to a function
(defun foo--register (name args)
  (let ((processed (some-complex-transformation args)))
    (register-foo name)
    (setup-foo name processed)))

(defmacro foo (name &rest args)
  `(foo--register ,name ,args))

;; bad - complex logic inlined in the macro body
(defmacro foo (name &rest args)
  (let ((processed (some-complex-transformation args)))
    `(progn
       (register-foo ',name)
...
- Prefer backquote (`` ` ``) over building lists manually with `list`
  and `cons`.
...
- Always declare the
  [debug-specification](http://www.gnu.org/software/emacs/manual/html_node/elisp/Specification-List.html#Specification-List),
...
- Declare the [indent
  specification](https://www.gnu.org/software/emacs/manual/html_node/elisp/Indenting-Macros.html#Indenting-Macros)
  if the macro arguments should not be aligned like a function (think of
  `defun` or `with-current-buffer`).
...
- **Do not** provide `autoload` cookies for non-definition top-level
  ...
  user's configuration). The single exception: `auto-mode-alist` can be
  altered for new major modes.
...
- Use [package-lint](https://github.com/purcell/package-lint) to check
  packages before submission to repositories such as
